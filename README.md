...
관리자 사이드바의 `회원 관리`에서는 이메일로 회원을 검색하고 가입일, 마지막 로그인,
관심 게임 수, 활성 가격 알림 수와 계정 상태를 확인할 수 있습니다. 일반 회원 계정은
정지·해제할 수 있으며 정지 시 기존 로그인 세션도 즉시 만료됩니다. 비밀번호는 관리자에게
공개하거나 직접 변경하지 않고 기존 비밀번호 재설정 메일만 요청할 수 있습니다. 관리자
계정 정지와 역할 변경은 안전을 위해 이 화면에서 제공하지 않으며 모든 조치는 감사 기록에
남습니다.

...
같은 화면의 `Steam 자동 동기화`는 Steam 인기작·할인작·신작 공개 목록을 소스별
3페이지까지 읽고 아직 처리하지 않은 항목을 작은 배치로 검사합니다. Steam에서
`game`으로 확인되고 제목이 명확한
일반판 본편만 자동 등록합니다. 한국어 제목만으로 canonical ID를 만들 수 없으면
동일 상품의 Steam 영문 제목을 한 번 더 조회해 canonical ID와 대표 제목으로
사용하고, 한국어 제목은 검색 가능한 alias로 보존합니다. Deluxe, DLC, Bundle
등으로 의심되는 항목은 계속 SQLite의 검토 대기열로 분리합니다. 처리 완료 App ID와
최근 실행 결과도 SQLite에 저장되므로 다음 실행은 중단 지점 이후의 미처리 항목을
계속 검사합니다. 과거에 한글 제목 때문에 검토 대기된 항목도 다음 동기화에서 자동으로
재검사합니다.

발견 순서는 사용자가 검색 결과 화면에서 요청한 게임, Steam 인기작·할인작·신작
순서입니다. 폐기된 `ISteamApps/GetAppList/v2`는 호출하지 않으므로 404가 전체
동기화 경고로 남지 않습니다. 검색 요청은 같은 이름을 대소문자나 공백만 바꿔 다시
...
자동 검사 결과는 운영 DB와 같은 volume의 `artwork-quality-status.json`에 원자적으로
저장됩니다. 관리자 대시보드에서 검사 후보, 자동 교체, 기존 유지, 깨짐/실패 수와 최근
판정 이유를 확인할 수 있습니다. 수동 교체 화면은 현재 이미지와 새 URL 미리보기를
나란히 표시하며, 저장 직후에는 직전 URL로 되돌릴 수 있습니다. 외부 URL이 응답하지
않거나 이미지 형식이 아니면 깨진 이미지로 기록되고 다음 후보를 사용합니다.
...
카탈로그 관리 화면은 `대시보드`, Store별 관리 화면, `데이터 정합성`, `변경 기록`으로
분리됩니다. Steam은 `새 후보 찾기 → 대기 후보 등록 → 가격 수집`, Nintendo·PlayStation·
Microsoft·Google Play·Apple App Store는 `상품 후보 찾기 → 검토 대기 → 가격 수집` 순서의
독립 버튼을 제공합니다. Epic Games Store는 서버 검색 제한 때문에 `공식 Store 검색 →
URL 검증·연결 → 가격 수집` 순서로 진행합니다. Store 선택 dropdown은 사용하지 않아 다른
Store에 잘못 연결하는 실수를 방지합니다.

관리자 사이드바의 `회원 관리`에서는 이메일 검색과 상태·권한 필터, 가입일·이메일 정렬,
페이지 이동을 사용할 수 있습니다. 회원 상세에서 가입일, 마지막 로그인, 관심 게임 수,
활성 가격 알림 수와 계정 상태를 확인합니다. 일반 회원 계정은 사유를 기록한 뒤 정지할 수
있고 정지 즉시 기존 세션도 만료됩니다. 정지 해제와 비밀번호 재설정 메일 발송도
지원합니다. 비밀번호 원문은 열람하지 않으며, 감사 기록에는 대상 회원과 처리 관리자
이메일 및 조치 사유가 표시됩니다. 관리자 계정 정지와 역할 변경은 안전을 위해 이 화면에서
제공하지 않습니다.

관리자 대시보드는 Store별 등록 상품, 최신·오래된 가격, 검토 대기 건수, 최근
카탈로그 처리·자동 등록·제외·실패 수, 최근 7일 추가 수와 마지막 성공 수집 시간을
한 화면에 표시합니다. 모바일 Store 화면은 최근 실행의 주요 판정 사유를 건수로
보여주므로 검색 결과 부족과 title/developer 불일치를 구분할 수 있습니다. 상품 연결
직후 잘못된 매칭을 발견하면
...
모바일 신규 등록은 유료 게임만 자동 허용합니다. `mobile_catalog_growth.py`는
Apple 한국 유료 게임 순위(최대 100개)와 순환 검색어, Google Play 순환 검색어에서
후보를 찾고 상세 페이지의 게임 분류·KRW 양수 가격·기기·개발사 정보를 검증합니다.
무료 게임은 기존 게임과 제목/alias 및 개발사·퍼블리셔가 일치할 때만 연결하며,
모바일 전용 무료 게임·데모·체험판·공략·동반 앱은 신규 등록하지 않습니다.
유사 제목은 자동 병합하지 않고 `mobile_growth_candidates`의 `NEEDS_REVIEW`로
보류합니다(현재 관리자 검토 화면이 아닌 DB에 사유/메타데이터를 보관).
한국어/영어 제목이 완전히 다른 상품은 같은 게임으로 자동 판별하지 못할 수 있습니다.
`catalog-maintenance` 주기마다 Store별 최대 `MOBILE_CATALOG_BATCH_SIZE`개(기본 50,
범위 1~100)를 처리합니다. 기본 주기는 6시간이며 전체 모바일 카탈로그 탐색은 아닙니다.
새 상품은 최초 가격 수집까지 실행하고 DB의 실제 갱신 시각을 확인합니다.
최초 가격 실패는 저장된 대기 상태로 다음 날 재시도하며 기존 일일 가격 갱신도 적용됩니다.
출력 보고서는 `registered`, `linked`, `review`, `excluded`, `failed`, `priceFailed`를
구분합니다. 무료 모바일 다운로드는 별도 안내로 표시하고 본편 최저가, 비교 그래프,
무료 전환에 따른 가격 알림에서는 제외합니다. 기존 카탈로그를 일괄 삭제하지 않습니다.

웹의 게임 목록과 상세 화면은 분리되어 있습니다. 검색·카테고리 결과에서 게임을
선택하면 `/games/{gameId}`로 이동하며, 플랫폼 필터는 상세 URL의 `platform` query로
표현됩니다. 브라우저 뒤로 가기 또는 `게임 목록으로`를 사용하면 이전 검색 조건,
정렬, 페이지와 스크롤 위치를 복원합니다. 존재하지 않는 게임 URL은 복구 가능한
오류 화면을 표시합니다.
...
자동 실행합니다. `catalog-growth`는 대기 후보를 연속 등록하고 첫 가격을 수집합니다.
...
한 잠금 구간에서 최대 200개 **후보**를 검사하며, 배치를 모두 처리하고 등록 검사 오류가
없으면 잠금을 해제한 뒤 5초 후 다음 구간을 실행합니다. 시간당 200개 제한은 없습니다.
후보가 소진되거나 검사 오류가 있으면 설정된 주기로 돌아갑니다. 실행 시각이 된 가격
갱신 작업은 예약 잠금을 유지해 신규 등록보다 먼저 실행됩니다.
실제 등록 수는 본편 판정·중복·검토 여부에 따라
달라집니다. Steam 공개 검색 목록의 다음 페이지를 저장해 재시작 후 이어서 탐색하고,
등록 대기 후보가 1,000개 이상이면 먼저 대기열을 처리합니다. 현재 신규 canonical
게임 탐색은 Steam 중심이며 콘솔 작업은 기존 게임에 상품을 연결합니다.
모바일은 기존 연결 작업 외에 위의 유료 게임 신규 탐색을 maintenance 주기에서 수행합니다.

신규 등록 가격 수집은 새 상품만 대상으로 합니다. 등록 응답에 유효한 KRW 본편 가격이
있고 조회한 지 10분 이내이면 같은 응답을 가격 파이프라인에서 재사용합니다. 누락·만료·
외화 응답은 기존 수집 경로로 조회하고, DB에 실제 가격이 저장됐는지 검증합니다.
카탈로그는 배치당 한 번 원자적으로 저장하며 상품별 감사 기록은 유지합니다.
등록 보고서의 `metadataFetchSeconds`, `catalogSaveSeconds`, `catalogBatchWrites`,
`reusablePrices`, `initialPriceSeconds`로 병목과 재사용 가능 가격 수를 확인합니다.
캐시 응답에는 네트워크 요청 간격 대기를 적용하지 않습니다. 자동 Steam 하위 배치는
`--skip-database-backup`으로 매 배치마다 전체 DB를 복사하지 않습니다. 별도
`backup-scheduler`와 배포 전 백업은 그대로 유지하며, 수동 Steam 파이프라인의 기본
백업 동작도 변경하지 않습니다.
성공한 가격은 재시도 목록에서 개별 제거하고,
...
공통 Store 가격 수집기는 429의 `Retry-After`(초 또는 HTTP 날짜)를 준수하고 요청 간격을
늘립니다. 20번 연속 성공하면 설정된 기본 속도까지 점진 복구합니다. 30초가 넘는
대기 또는 마지막 시도의 429는 남은 해당 배치를 실패/보류 처리하여 장시간 묶지 않습니다.
401/403도 같은 배치의 후속 요청을 중단하며 다른 Store 수집은 계속합니다.
...
정기 실행 순서는 Steam 인기·할인·인기 신작·신작·출시 예정 발견, 카탈로그 신규 상품 제한 배치 검사, 전체 Steam 가격 수집,
...
구분한다. Nintendo, PlayStation, Microsoft Store는 공식 Store 카탈로그에서 후보를 제한된 배치로
...
Microsoft Store 후보 탐색은 Xbox 공식 검색과 한국 Display Catalog를 사용한다.
`Game` 상품 중 Xbox One 또는 Xbox Series X|S를 지원하고 KRW 구매 가격이 확인된
상품만 자동 연결 대상으로 삼으며, DLC·소모품과 구독 라이선스는 제외한다.
...
`/api/games`는 가격 이력·구매 추천 없이 목록용 현재 가격만 조회합니다. 제목순의
Store/기기 미지정 목록은 페이지를 먼저 나누며, 전체 비교가 필요한 정렬·필터는
최대 500개 게임 단위로 가격을 묶어 조회합니다.
검색 결과는 API 프로세스 안에서 최대 10초·64개 조건으로 캐시합니다. DB 변경
(외부 collector 포함) 또는 카탈로그 reload 시 기존 캐시는 사용하지 않습니다.
브라우저 HTTP 캐시는 `no-store`이며, 응답의 `X-Search-Cache`는 HIT/MISS를 표시합니다.
`Server-Timing`의 `app`은 검색 핸들러 소요 시간, `db`는 가격 조회·비교 단계의 시간입니다.
네트워크 전송·화면 렌더링·핸들러 전 카탈로그 reload 시간은 포함하지 않습니다.
API 로그의 `catalog_search`에는 같은 시간과 반환/전체 개수가 기록되며 검색어나
사용자 정보는 기록하지 않습니다.

첫 목록은 12개, 자동완성은 6개만 요청합니다. 새 목록/상세 요청과 검색어 변경 시
이전 요청을 AbortController로 취소합니다. 이미 시작된 서버 SQL까지 중단하는 것은
아닙니다. 첫 카드와 상세 이미지는 우선 로드하고 나머지는 기존 지연 로딩을 유지합니다.
이미지 원본 URL·비율은 유지하며 별도 썸네일 변환 서버는 아직 사용하지 않습니다.

검색 UI 회귀 테스트만 실행하려면:

```sh
bash tests/web_ui_e2e.sh ./build/game_price_tracker ./build/game_price_api search-performance.spec.ts
```
...
게임을 선택하면 `/games/hollow-knight` 형태의 전용 상세 페이지로 이동하므로 특정 게임
화면을 북마크하거나 공유할 수 있습니다. 가격 카드의 Store 링크는 API가 제공한
공식 상품 페이지를 새 탭으로 엽니다.
플랫폼 버튼을 선택하면 해당 플랫폼을 지원하는 가격 카드와 그래프만 남고,
`/games/hades?platform=Windows`처럼 선택 상태가 공유 주소에도 포함됩니다.

...
export const importStorefrontCatalogGame = (store: 'EpicGamesStore' | 'NintendoEShop' | 'PlayStationStore' | 'MicrosoftStore' | 'UbisoftStore', productUrl: string, gameId: string, apply: boolean, acknowledgeReview = false) => requestJson<CatalogAdminResult>('/api/admin/catalog/storefront', { method: 'POST', body: JSON.stringify({ store, productUrl, gameId, apply, acknowledgeReview }) })

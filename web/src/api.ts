...
export const getAdminUsers = (
  ...
  const parameters = new URLSearchParams({ q: query, status, role, sort, page: String(page), pageSize: '20' })
  return requestJson<AdminUserPage>(`/api/admin/users?${parameters}`)
}
export const getAdminUser = (userId: number) => requestJson<AdminUser>(`/api/admin/users/${userId}`)
export const updateAdminUserStatus = (userId: number, active: boolean, reason = '') => requestJson<AdminUser>(`/api/admin/users/${userId}/status`, { method: 'PATCH', body: JSON.stringify({ active, ...(reason ? { reason } : {}) }) })
...
export const disconnectCatalogProduct = (store: string, productId: string) => requestJson<{ gameId: string; title: string; removedGame: boolean; applied: boolean }>(`/api/admin/catalog/products/${encodeURIComponent(store)}/${encodeURIComponent(productId)}`, { method: 'DELETE' })
...
export const importStorefrontCatalogGame = (store: 'EpicGamesStore' | 'NintendoEShop' | 'PlayStationStore' | 'MicrosoftStore' | 'UbisoftStore' | 'GOG' | 'MetaQuestStore' | 'EAApp' | 'BattleNet' | 'ItchIo' | 'HumbleStore', productUrl: string, gameId: string, apply: boolean, acknowledgeReview = false) => requestJson<CatalogAdminResult>('/api/admin/catalog/storefront', { method: 'POST', body: JSON.stringify({ store, productUrl, gameId, apply, acknowledgeReview }) })
export const getCatalogCollectionJob = () => requestJson<CatalogCollectionJob>('/api/admin/catalog/collection')
export const startCatalogCollection = (store = 'Steam', productId?: string) => requestJson<CatalogCollectionJob>('/api/admin/catalog/collection', { method: 'POST', body: JSON.stringify({ store, ...(productId ? { productId } : {}) }) })

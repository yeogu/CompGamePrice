...
배포 스크립트는 새 이미지를 만들기 전에 방금 생성한 SQLite 백업을 임시 경로에 실제로
복원해 무결성을 확인한다. 컨테이너 재생성 후에는 API health뿐 아니라 Web 빌드 revision도
검사하므로 이전 정적 파일이 계속 제공되면 배포를 성공으로 처리하지 않는다.
...
Microsoft Store는 Xbox 공식 검색과 한국 상품 카탈로그에서 후보를 자동 탐색한다.
`Game` 상품이며 KRW 구매 가격이 확인된 경우에만 자동 연결하고 DLC·소모품·구독
라이선스는 제외한다. Store와 실제
...
관리자 화면의 Nintendo eShop, PlayStation Store 또는 Microsoft Store 탭에서 `후보 배치 탐색`을 한 번 실행한 뒤, 최근 실행의
처리·자동 등록·검토·실패 수가 표시되는지 확인한다. 운영 catalog를 직접 수정하지 않는
사전 검증은 catalog와 DB 복사본에 `sync_mobile_catalog.py --store NintendoEShop`을
실행하는 방식으로 수행한다. PlayStation은 `--store PlayStationStore`, Microsoft는
`--store MicrosoftStore`를 사용한다.
...
- 회원 검색의 상태·권한 필터, 정렬, 페이지 이동이 정상인지 확인
- 계정 정지 시 사유 입력이 필수인지, 상세와 감사 기록에 같은 사유가 표시되는지 확인
- 감사 기록에 대상 회원 및 처리 관리자 이메일이 표시되는지 확인

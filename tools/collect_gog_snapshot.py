#!/usr/bin/env python3
"""Collect registered GOG prices from the official catalog API."""
...
def targets(catalog: Path) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        if product.get("store") == "GOG"
    ]


def fetch(product_id: str, game_id: str, product_url: str, timeout: float) -> bytes:
    del game_id, product_url
    return storefront_catalog.fetch_gog_product(product_id, timeout)


def normalized_block(raw: bytes, product_id: str, game_id: str, product_url: str) -> str:
    try:
        metadata = storefront_catalog.verified_product(raw, "GOG", product_url)
    except ValueError as error:
        raise support.PermanentCollectionError(str(error)) from error
    current = metadata.get("priceMinor")
    regular = metadata.get("regularPriceMinor") or current
    if current is None or regular is None:
        raise support.PermanentCollectionError("GOG product has no price")
    if current < 0 or regular < current:
        raise support.PermanentCollectionError("GOG product has an invalid price")
    os_names = {"Windows": "WIN", "macOS": "MAC", "Linux": "LINUX"}
    compatible = "|".join(
        os_names[name] for name in metadata["platforms"] if name in os_names)
    return "\n".join([
        f"offer_id: {product_id}",
        f"game_id: {game_id}",
        f"regular_price_minor: {regular}",
        f"current_price_minor: {current}",
        f"currency: {metadata['currency']}",
        f"discount_percent: {metadata.get('discountPercent', 0)}",
        f"compatible_os: {compatible}",
        "status: ACTIVE",
    ])
...
def main() -> int:
    ...
    parser.add_argument("--output", default=root / "snapshots/latest/gog_products.txt", type=Path)
    args = parser.parse_args()
    collected, failures = collect(args.catalog, args.output)
    print(f"Collected {collected} GOG products")

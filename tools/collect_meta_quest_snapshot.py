#!/usr/bin/env python3
"""Collect registered Meta Quest Store KR prices from official pages."""
...
def targets(catalog: Path) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        if product.get("store") == "MetaQuestStore"
    ]


def fetch(product_id: str, game_id: str, product_url: str, timeout: float) -> bytes:
    del product_id, game_id
    return storefront_catalog.fetch_product("MetaQuestStore", product_url, timeout)
...
def normalized_block(raw: bytes, product_id: str, game_id: str, product_url: str) -> str:
    try:
        metadata = storefront_catalog.verified_product(
            raw, "MetaQuestStore", product_url)
    except ValueError as error:
        raise support.PermanentCollectionError(str(error)) from error
    price = metadata.get("priceMinor")
    if price is None:
        raise support.PermanentCollectionError("Meta Quest product has no price")
    if metadata.get("currency") != "KRW":
        raise support.PermanentCollectionError("Meta Quest price must be KRW")
    if price < 0:
        raise support.PermanentCollectionError("Meta Quest price cannot be negative")
    return "\n".join([
...
def main() -> int:
    ...
    parser.add_argument("--output", default=root / "snapshots/latest/meta_quest_products.txt", type=Path)
    args = parser.parse_args()
    collected, failures = collect(args.catalog, args.output)
    print(f"Collected {collected} Meta Quest Store products")

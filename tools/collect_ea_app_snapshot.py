#!/usr/bin/env python3
"""Collect registered EA app KR prices from official EA buy pages."""
...
def normalized_block(raw: bytes, product_id: str, game_id: str, product_url: str) -> str:
    ...
    if price is None or regular is None:
        raise support.PermanentCollectionError("EA app product has no price")
    if currency not in {"KRW", "USD", "EUR", "GBP", "JPY"}:
        raise support.PermanentCollectionError("EA app currency is unsupported")
    if price < 0 or regular < price:
        raise support.PermanentCollectionError("EA app price is invalid")
    return "\n".join([
...
def main() -> int:
    ...
    parser.add_argument("--output", default=root / "snapshots/latest/ea_app_products.txt", type=Path)
    args = parser.parse_args()
    collected, failures = collect(args.catalog, args.output)
    print(f"Collected {collected} EA app products")
